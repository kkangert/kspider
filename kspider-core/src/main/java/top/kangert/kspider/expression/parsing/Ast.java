...
package top.kangert.kspider.expression.parsing;

import top.kangert.kspider.expression.DynamicMethod;
...
import top.kangert.kspider.expression.interpreter.JavaReflection;
...
/**
 * Templates are parsed into an abstract syntax tree (AST) nodes by a Parser.
 * This class contains all AST node types.
 */
public abstract class Ast {

    /**
     * Base class for all AST nodes. A node minimally stores the {@link Span} that
     * references its location in the
     * {@link Source}.
     **/
    public abstract static class Node {
        ...
        /**
         * Returns the {@link Span} referencing this node's location in the
         * {@link Source}.
         **/
        public Span getSpan() {
            return span;
        ...
        public abstract Object evaluate(ExpressionTemplate template, ExpressionTemplateContext context)
                throws IOException;
    }

    /**
     * A text node represents an "un-templated" span in the source that should be
     * emitted verbatim.
     **/
    ...
    /**
     * All expressions are subclasses of this node type. Expressions are separated
     * into unary operations (!, -), binary operations
            ...
            if (getOperator() == UnaryOperator.Negate) {
                if (operand instanceof Integer) {
                    ...
                    ExpressionError.error(
                            "Operand of operator '" + getOperator().name() + "' must be a number, got " + operand,
                            getSpan());
                    return null; // never reached
                }
            } else if (getOperator() == UnaryOperator.Not) {
                if (!(operand instanceof Boolean)) {
                    ExpressionError.error("Operand of operator '" + getOperator().name() + "' must be a boolean",
                            getSpan());
                }
    ...
    /**
     * A binary operation represents arithmetic operators, like addition or
     * division, comparison operators, like less than or
     * equals, logical operators, like and, or an assignment.
     **/
    public static class BinaryOperation extends Expression {

        public static enum BinaryOperator {
            Addition, Subtraction, Multiplication, Division, Modulo, Equal, NotEqual, Less, LessEqual, Greater,
            GreaterEqual, And, Or, Xor, Assignment;
        ...
        private Object evaluateAddition(Object left, Object right) {
            ...
            ExpressionError.error(
                    "Operands for addition operator must be numbers or strings, got " + left + ", " + right + ".",
                    getSpan());
            return null; // never reached
        }
        ...
        private Object evaluateSubtraction(Object left, Object right) {
            if (left instanceof Double || right instanceof Double) {
                ...
                ExpressionError.error("Operands for subtraction operator must be numbers" + left + ", " + right + ".",
                        getSpan());
                return null; // never reached
            }
        }
        ...
        private Object evaluateMultiplication(Object left, Object right) {
            if (left instanceof Double || right instanceof Double) {
                ...
                ExpressionError.error(
                        "Operands for multiplication operator must be numbers" + left + ", " + right + ".", getSpan());
                return null; // never reached
            }
        }
        ...
        private Object evaluateDivision(Object left, Object right) {
            if (left instanceof Double || right instanceof Double) {
                ...
                ExpressionError.error("Operands for division operator must be numbers" + left + ", " + right + ".",
                        getSpan());
                return null; // never reached
            }
        }
        ...
        private Object evaluateModulo(Object left, Object right) {
            if (left instanceof Double || right instanceof Double) {
                ...
                ExpressionError.error("Operands for modulo operator must be numbers" + left + ", " + right + ".",
                        getSpan());
                return null; // never reached
            }
        }
        ...
        private boolean evaluateLess(Object left, Object right) {
            if (left instanceof Double || right instanceof Double) {
                ...
                ExpressionError.error("Operands for less operator must be numbers" + left + ", " + right + ".",
                        getSpan());
        ...
        private Object evaluateLessEqual(Object left, Object right) {
            if (left instanceof Double || right instanceof Double) {
                ...
                ExpressionError.error("Operands for less/equal operator must be numbers" + left + ", " + right + ".",
                        getSpan());
                return null; // never reached
            }
        }
        ...
        private Object evaluateGreater(Object left, Object right) {
            ...
            if (left instanceof Double || right instanceof Double) {
                ...
                ExpressionError.error("Operands for greater operator must be numbers" + left + ", " + right + ".",
                        getSpan());
                return null; // never reached
            }
        }
        ...
        private Object evaluateGreaterEqual(Object left, Object right) {
            if (left instanceof Double || right instanceof Double) {
                ...
                ExpressionError.error("Operands for greater/equal operator must be numbers" + left + ", " + right + ".",
                        getSpan());
                return null; // never reached
            }
        }

        private Object evaluateAnd(Object left, ExpressionTemplate template, ExpressionTemplateContext context)
                throws IOException {
            ...
            if (!(right instanceof Boolean)) {
                ExpressionError.error("Right operand must be a boolean, got " + right + ".",
                        getRightOperand().getSpan());
            }
            return (Boolean) left && (Boolean) right;
        }

        private Object evaluateOr(Object left, ExpressionTemplate template, ExpressionTemplateContext context)
                throws IOException {
            if (!(left instanceof Boolean)) {
                ExpressionError.error("Left operand must be a boolean, got " + left + ".", getLeftOperand().getSpan());
            }
        ...
        public Object evaluate(ExpressionTemplate template, ExpressionTemplateContext context) throws IOException {
            if (getOperator() == BinaryOperator.Assignment) {
                if (!(getLeftOperand() instanceof VariableAccess)) {
                    ExpressionError.error("Can only assign to top-level variables in context.",
                            getLeftOperand().getSpan());
                }
            ...
            Object right = getOperator() == BinaryOperator.And || getOperator() == BinaryOperator.Or ? null
                    : getRightOperand().evaluate(template, context);
    ...
    /**
     * A ternary operation is an abbreviated if/then/else operation, and equivalent
     * to the the ternary operator in Java.
     **/
    public static class TernaryOperation extends Expression {
        ...
        public Object evaluate(ExpressionTemplate template, ExpressionTemplateContext context) throws IOException {
            ...
            if (!(condition instanceof Boolean)) {
                ExpressionError.error("Condition of ternary operator must be a boolean, got " + condition + ".",
                        getSpan());
            }
            return ((Boolean) condition) ? getTrueExpression().evaluate(template, context)
                    : getFalseExpression().evaluate(template, context);
        }
    }
    ...
    /**
     * A double precision floating point literal. Must be marked with the
     * <code>d</code> suffix, e.g. "1.0d".
     **/
    ...
    /**
     * A single precision floating point literla. May be optionally marked with the
     * <code>f</code> suffix, e.g. "1.0f".
     **/
    ...
    /**
     * A long integer literal. Must be marked with the <code>l</code> suffix, e.g.
     * "123l".
     **/
    ...
    /**
     * Represents a top-level variable access by name. E.g. in the expression "a +
     * 1", <code>a</code> would be encoded as a
     * VariableAccess node. Variables can be both read (in expressions) and written
     * to (in assignments). Variable values are looked
    ...
    public static class VariableAccess extends Expression {
        ...
        public Object evaluate(ExpressionTemplate template, ExpressionTemplateContext context) throws IOException {
            ...
            // if (value == null) ExpressionError.error("找不到变量'" + getSpan().getText() +
            // "'或变量值为null", getSpan());
            return value;
        }
    }
    ...
    /**
     * Represents a map or array element access of the form
     * <code>mapOrArray[keyOrIndex]</code>. Maps and arrays may only be read
     * from.
     **/
    public static class MapOrArrayAccess extends Expression {
        ...
        /**
         * Returns an expression that is used as the key or index to fetch a map or
         * array element.
         **/
        ...
        public Object evaluate(ExpressionTemplate template, ExpressionTemplateContext context) throws IOException {
            ...
            if (mapOrArray instanceof Map) {
                ...
                if (!(keyOrIndex instanceof Number)) {
                    ExpressionError.error(
                            "List index must be an integer, but was " + keyOrIndex.getClass().getSimpleName(),
                            getKeyOrIndex().getSpan());
                }
                ...
                if (!(keyOrIndex instanceof Number)) {
                    ExpressionError.error(
                            "Array index must be an integer, but was " + keyOrIndex.getClass().getSimpleName(),
                            getKeyOrIndex().getSpan());
                }
                int index = ((Number) keyOrIndex).intValue();
    ...
    /**
     * Represents an access of a member (field or method or entry in a map) of the
     * form <code>object.member</code>. Members may
     * only be read from.
     **/
    public static class MemberAccess extends Expression {
         ...
         * {@link AbstractReflection#getField(Object, String)} or
         * {@link AbstractReflection#getMethod(Object, String, Object...)}. See
         ...
         * {@link AbstractReflection#getField(Object, String)} or
         * {@link AbstractReflection#getMethod(Object, String, Object...)} for faster
         * member lookups. Called by {@link AstInterpreter} the
         * first time this node is evaluated. Subsequent evaluations can use the cached
         * descriptor, avoiding a costly reflective
         * lookup.
         **/
        ...
        public Object evaluate(ExpressionTemplate template, ExpressionTemplateContext context) throws IOException {
            ...
            if (field == null) {
                ...
                try {
                    ...
                    if (ExceptionUtil.isFromOrSuppressedThrowable(e, InvocationTargetException.class)) {
                        ExpressionError.error(String.format("在%s中调用方法get%s发生异常", object.getClass(), methodName),
                                getSpan(), e);
                        return null;
                    }
                    ...
                    try {
                        ...
                        if (ExceptionUtil.isFromOrSuppressedThrowable(e1, InvocationTargetException.class)) {
                            ExpressionError.error(String.format("在%s中调用方法is%s发生异常", object.getClass(), methodName),
                                    getSpan(), e);
                            return null;
                        }
                        ExpressionError.error(String.format("在%s中找不到属性%s或者方法get%s、方法is%s", object.getClass(),
                                getName().getText(), methodName, methodName), getSpan());
                    }
                }
            }
    ...
    public static class LambdaAccess extends Expression {
        ...
        public Object evaluate(ExpressionTemplate template, ExpressionTemplateContext context) throws IOException {
            if (ArrayLikeLambdaExecutor.SUPPORT_METHOD.contains(arrayLike.getName().getText())) {
                ...
                ExpressionError.error("只支持 " + String.join(",", ArrayLikeLambdaExecutor.SUPPORT_METHOD)
                        + "。不支持的lambda函数： " + arrayLike.getName().getText(), arrayLike.getSpan());
            }
            return null;
        }
        ...
        private <T, R> Object oneArgumentParser(ExpressionTemplate template, ExpressionTemplateContext context) {
            ...
            if (arrLikeObj.getClass().isArray()) {
                try {
                    ...
                    // arrLikeObj.getClass().getDeclaredField("length").get(arrLikeObj);
    ...
    /**
     * Represents a call to a top-level function. A function may either be a
     * {@link FunctionalInterface} stored in a
    ...
    public static class FunctionCall extends Expression {
        ...
        /**
         * Return the expression that must evaluate to a {@link FunctionalInterface} or
         * a {@link Macro}.
         **/
        public Expression getFunction() {
            return function;
        ...
        /**
         * Returns the cached "function" descriptor as returned by
         * {@link AbstractReflection#getMethod(Object, String, Object...)} or the
        ...
        /**
         * Sets the "function" descriptor as returned by
         * {@link AbstractReflection#getMethod(Object, String, Object...)} for faster
         * lookups, or the {@link Macro} to be called. Called by {@link AstInterpreter}
         * the first time this node is evaluated.
         * Subsequent evaluations can use the cached descriptor, avoiding a costly
         * reflective lookup.
         **/
         ...
         * {@link AstInterpreter}. Avoids generating
         * garbage.
         **/
        ...
        public Object evaluate(ExpressionTemplate template, ExpressionTemplateContext context) throws IOException {
            try {
                ...
                // This is a special case to handle template level macros. If a call to a macro
                // is
                // made, evaluating the function expression will result in an exception, as the
                // function name can't be found in the context. Instead we need to manually
                // check
    ...
    /**
     * Represents a call to a method of the form
     * <code>object.method(a, b, c)</code>.
     **/
    public static class MethodCall extends Expression {
         ...
         * {@link AbstractReflection#getMethod(Object, String, Object...)} for faster
         * lookups.
         * Called by {@link AstInterpreter} the first time this node is evaluated.
         * Subsequent evaluations can use the cached
            ...
            try {
                ...
                if (object instanceof DynamicMethod) {
                    try {
                        ...
                        Object[] newArgumentValues = new Object[] { getMethod().getName().getText(),
                ...
                if (method != null) {
                    try {
                        ...
                        // fall through
                        if (t.getCause() != null
                                && t.getCause().getCause() instanceof ArrayLikeLambdaExecutor.LambdaExecuteException) {
                            ArrayLikeLambdaExecutor.LambdaExecuteException lee = (ArrayLikeLambdaExecutor.LambdaExecuteException) t
                                    .getCause().getCause();
                        ...
                        } else if (isCachedMethodStatic() && RuntimeException.class.equals(t.getClass())
                                && t.getCause() != null) {
                ...
                method = AbstractReflection.getInstance().getMethod(object, getMethod().getName().getText(),
                        argumentValues);
                if (method != null) {
                ...
                method = AbstractReflection.getInstance().getExtensionMethod(object, getMethod().getName().getText(),
                        argumentValues);
                if (method != null) {
                    try {
                    ...
                    // didn't find the method on the object, try to find a field pointing to a
                    // lambda
                    Object field = AbstractReflection.getInstance().getField(object, getMethod().getName().getText());
                    if (field == null) {
                        Iterator<String> iterator = Arrays.asList(JavaReflection.getStringTypes(argumentValues))
                                .iterator();
                        ExpressionError.error(
                                "在'" + object.getClass() + "'中找不到方法 " + getMethod().getName().getText() + "("
                                        + StrUtil.join(",", iterator) + ")",
                                getSpan());
                    }
    ...
    /**
     * Represents a map literal of the form
     * <code>{ key: value, key2: value, ... }</code> which can be nested.
     */
    ...
    /**
     * Represents a list literal of the form
     * <code>[ value, value2, value3, ...]</code> which can be nested.
     */
